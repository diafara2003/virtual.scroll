interface fn_chunked {
  ...
  class VirtualScroll {
    ...
    _init(): void {
      ...
      this._v_object._html += this._setting.fn_chunked(
        this._v_object._inital,
        this._v_object._from,
        this._setting.source
      );
      this._v_object._html += this._tr_scroll(
        tr_ultimo,
        _missing * this._setting._length_tr
      );
      ...
      let _visor_scrollind: HTMLElement | null = document.getElementById(
        this._setting.id_visor
      );
    ...
    _tr_scroll(id: string, _length: number): string {
      let _html_tr = "";
      _html_tr =
        "<tr><td id=" + id + ' style="height:' + _length + 'px"></td></tr>';
    ...
    _get_length_viewver(): number {
      let _el: HTMLElement | null = document.getElementById(
        this._setting.id_visor
      );
      if (_el != null) {
    ...
    _get_top(): number {
      let result: number = Math.floor(
        this._v_object._scroll / this._setting._length_tr
      );
    ...
    _get_bottom(): number {
      let result: number =
        this._v_object._from - (this._get_top() + this._get_length_viewver());
    ...
    _scrolling(_scrolled: number): void {
      ...
      if (_container != this._v_object._container) {
        ...
        let _inital =
          this._v_object._from * (this._v_object._container - 1) -
          this._get_length_viewver();
        let _missing: number = this._v_object._length_Datos - _from;
  
        if (_inital < 0 || _scrolled==0) {
          _inital = 0;
        ...
        this._v_object._html += this._tr_scroll(
          tr_primero,
          _inital * this._setting._length_tr
        );
        this._v_object._html += this._setting.fn_chunked(
          _inital,
          _from,
    ...
    _get_container(): number {
      ...
      if (result.toString().split(".").length == 2) {
        let _decimal: number = parseFloat(
          result
            .toString()
            .split(".")[1]
            .substr(0, 1)
